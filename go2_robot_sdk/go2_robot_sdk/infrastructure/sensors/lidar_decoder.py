...
def update_meshes_for_cloud2(
    ...
    if voxel_leaf and voxel_leaf > 0.0 and len(filtered_points) > 0:
        # Dedup theo lưới voxel. np.unique(axis=0) sort toàn mảng (~70ms/150k điểm).
        # Gộp 3 chỉ số voxel thành 1 khoá int64 (lưới bị chặn -> không tràn/collision)
        # rồi np.unique 1D -> exact & nhanh hơn ~4x, giảm chặn event loop/GIL.
...
class LidarDecoder:
    ...
    def copy_within(self, target, start, end):
        # Bulk C-speed move (memmove xử lý cả vùng chồng lấn). Trước đây copy
        # từng byte bằng vòng lặp Python -> chặn event loop asyncio khi decode.
        heap_len = len(self.HEAPU8)
        n = end - start
        if n <= 0:
            return
        if target + n > heap_len:
            n = heap_len - target
        if n <= 0:
            return
        base = ctypes.addressof(self.HEAPU8)
        ctypes.memmove(base + target, base + start, n)
    ...
    def add_value_arr(self, start, value):
        # Bulk C-speed copy vào bộ nhớ WASM. Trước đây copy từng byte bằng vòng
        # lặp Python (~60KB/frame) -> chặn event loop -> camera/scan lag.
        n = len(value)
        if start + n > len(self.HEAPU8):
            raise ValueError("Not enough space to insert bytes at the specified index.")
        ctypes.memmove(ctypes.addressof(self.HEAPU8) + start, bytes(value), n)
    ...
    def decode(self, compressed_data, data):
        ...
        # Đọc khối C-speed từ bộ nhớ WASM. Trước đây slice ctypes -> list Python
        # (chậm, giữ GIL). .copy() vì vùng nhớ WASM bị ghi đè ở frame kế.
        p = np.frombuffer(self.HEAPU8, dtype=np.uint8,
                          count=u * 12, offset=self.positions).copy()
        r = np.frombuffer(self.HEAPU8, dtype=np.uint8,
                          count=u * 8, offset=self.uvs).copy()
        o = np.frombuffer(self.HEAPU8, dtype=np.uint8,
                          count=u * 24, offset=self.indices).copy().view(np.uint32)
