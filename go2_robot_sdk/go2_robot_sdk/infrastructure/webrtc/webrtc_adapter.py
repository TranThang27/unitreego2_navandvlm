...
class WebRTCAdapter(IRobotDataReceiver, IRobotController):
    ...
    async def connect(self, robot_id: str) -> None:
        ...
        try:
            ...
            await conn.connect()
            # disable_traffic_saving được gửi trong _on_validated (lúc data channel
            # đã mở); gọi ở đây quá sớm -> kênh chưa mở -> no-op.
    ...
    def _on_validated(self, robot_id: str) -> None:
        ...
        try:
            if robot_id in self.connections:
                ...
                # Bật disable_traffic_saving (robot khuyến cáo khi subscribe lidar):
                # ưu tiên băng thông data -> luồng ổn định hơn. Kênh đã mở ở đây.
                self.connections[robot_id].data_channel.send(json.dumps({
                    "type": "rtc_inner_req",
                    "topic": "",
