...
class Go2DriverNode(Node):
    ...
    def __init__(self, event_loop=None):
        ...
        # Xử lý dữ liệu robot (dựng PointCloud2 ~60ms/frame) chạy trên 1 thread
        # RIÊNG, không phải event loop asyncio. aiortc nhận video trên event loop;
        # nếu xử lý lidar chạy trên loop nó sẽ đói video -> camera/scan lag. numpy
        # nhả GIL nên thread thật sự giải phóng loop. 1 worker -> giữ đúng thứ tự.
        from concurrent.futures import ThreadPoolExecutor
        self._data_executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="robotdata")
    ...
    def _on_robot_data_received(self, msg: Dict[str, Any], robot_id: str) -> None:
        ...
        Gọi từ event loop asyncio (data-channel callback). Đẩy sang thread riêng
        để việc dựng PointCloud2 KHÔNG chặn loop -> video (aiortc) hết bị đói.
        """
