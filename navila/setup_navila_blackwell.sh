...
#   - bitsandbytes >=0.45 (dùng 8-bit; KHÔNG 4-bit vì bnb 4-bit ra NaN trên Blackwell) + vá fp16_statistics
#   - modeling_llama: decoder eager + rotary API mới + MASK NHÂN-QUẢ 4D (eager bắt buộc, bản gốc chỉ hợp flash)
...
def _sdpa(q, k, v, dropout_p=0.0, softmax_scale=None, causal=False):
    ...
    if q.shape[1] != k.shape[1]:               # GQA: nhân bản KV head cho khớp Q head
        rep = q.shape[1] // k.shape[1]
        k = k.repeat_interleave(rep, dim=1); v = v.repeat_interleave(rep, dim=1)
    attn_mask, is_causal = None, False
    if causal:                                 # decode có cache (Sq!=Sk): căn bottom-right, không dùng is_causal
        Sq, Sk = q.shape[2], k.shape[2]
        if Sq == Sk:
            is_causal = True
        else:
            attn_mask = torch.ones(Sq, Sk, dtype=torch.bool, device=q.device).tril(diagonal=Sk - Sq)
    o = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=dropout_p, is_causal=is_causal, scale=softmax_scale)
...
# 6e. modeling_llama: LlamaModel.forward dựng MASK NHÂN-QUẢ 4D cho eager
#     (bản gốc chỉ truyền mask padding 2D/None vì viết cho flash -> eager nhìn 2 chiều -> output degenerate)
patch(ml,
'''        attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None''',
'''        from transformers.modeling_attn_mask_utils import _prepare_4d_causal_attention_mask  # PATCH: eager cần mask 4D
        attention_mask = _prepare_4d_causal_attention_mask(
            attention_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length
        )''',
"llama 4D causal mask (eager)")
...
echo -e "\n\033[1;32m== XONG. Chạy thử (8-bit, KHÔNG --load-4bit): ==\033[0m"
echo "cd $NAVILA_DIR && CUDA_HOME=\$HOME/miniconda3/envs/$ENV PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \\"
echo "  conda run -n $ENV --no-capture-output python ~/ros2_vlm/src/navila/navila_verify.py --model-path $CKPT_DIR"
echo "Kỳ vọng RAW: 'The next action is move forward 25 cm.' (nếu ra dấu chấm/rác => patch mask 4D chưa vào)"
